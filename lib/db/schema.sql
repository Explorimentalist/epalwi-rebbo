...
-- Compatible with existing UserProfile and auth types
...
-- Trial periods table - 14-day free trial tracking
CREATE TABLE trials (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    ...
    days_remaining INTEGER GENERATED ALWAYS AS (
        CASE 
            WHEN end_date > CURRENT_TIMESTAMP 
            THEN EXTRACT(DAY FROM end_date - CURRENT_TIMESTAMP)::INTEGER + 1
            ELSE 0
        END
    ) STORED,
    is_expired BOOLEAN GENERATED ALWAYS AS (end_date <= CURRENT_TIMESTAMP) STORED,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);
...
CREATE INDEX idx_trials_expired ON trials(is_expired);
...
-- Active users with valid access (trial or subscription)
CREATE VIEW active_users_with_access AS
SELECT 
    u.id,
    u.email,
    u.display_name,
    ...
    u.last_login_at,
    CASE 
        WHEN t.is_expired = false THEN true
        WHEN s.status = 'active' THEN true
        ELSE false
    END as has_access,
    CASE
        WHEN t.is_expired = false THEN 'trial'
        WHEN s.status = 'active' THEN 'subscription'
        ELSE 'expired'
    END as access_type,
    t.days_remaining as trial_days_remaining,
